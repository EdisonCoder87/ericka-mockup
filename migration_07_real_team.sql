...
--    Temp PINs — each person changes theirs on first login via "Change my PIN".

-- Owner tier: Grace (full admin, sees everything)
insert into users (name, pin, role, client_id, vertical, billable_rate, pay_rate, active)
select 'Grace','6041','admin', null, null, 0, 0, true
where not exists (select 1 from users where name = 'Grace');

-- Client tier: Rad (SIA Medical, view-only)
insert into users (name, pin, role, client_id, vertical, billable_rate, pay_rate, active)
select 'Rad (SIA Medical)','7052','client_admin',
       '11111111-1111-1111-1111-111111111111','medical',0,0,true
where not exists (select 1 from users where name = 'Rad (SIA Medical)');

-- Ericka Manager tier: Shane & Sharica (manage the whole remote team).
-- client_id null = whole-team reach (not bound to one client).
insert into users (name, pin, role, client_id, vertical, billable_rate, pay_rate, active)
select 'Shane','4821','manager', null, null, 0, 0, true
where not exists (select 1 from users where name = 'Shane');

insert into users (name, pin, role, client_id, vertical, billable_rate, pay_rate, active)
select 'Sharica','5932','manager', null, null, 0, 0, true
